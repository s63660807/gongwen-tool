...
**落款被挤到单独一页时**，加 `--fit-signoff` 做紧凑处置（GB/T 9704-2012 允许「调整行距、字间距，
务使印章与正文同处一页」）：

```bash
<python> <skill_dir>/scripts/fix_gongwen_format.py <输入.md> [输出.docx] --fit-signoff[=1|2]
```

- `=1`（默认）：取消表格后段前间距 + 收紧表格单元格上下内边距与表内行距，约省 30~40pt
- `=2`：在 1 之上再把正文行距由 28 磅压到 27 磅，每页约再省 22pt
- 先试 1，转 PDF 看落款是否已回正文页；不够再上 2
...
| 跨页 | 表头行 `tblHeader` 跨页自动重复；单行 `cantSplit` 不拆行；表格前段落 `keepNext` + 除末行外各行段落 `keepNext`，保证**整张表不拆到两页** |
...
19. **表格防跨页四件套**：`tblHeader`（表头跨页重复）+ 行 `cantSplit`（单行不被拆）+ 表格前段落 `keepNext`（表格不从页面底部挤起）+ **除末行外各行的段落都加 `keepNext`**（行行粘连，整张表一起换页）。前三条只保证「单行不拆、表头重复」，**整表仍可能被拆成「页尾只留表头、数据行翻页」**——实测踩到过。第 4 条才真正让整表同页；表格本身高于一页时 Word 会自动放弃该约束，不会报错。
   - **`keepNext` 的插入位置有讲究**：CT_PPr schema 要求它紧跟 `w:pStyle`。直接 `append` 到 pPr 末尾会得到不合 schema 顺序的 XML（Word/WPS 通常容错，但不保险）。正确做法：找到 `w:pStyle` 后 `addnext`，无 `pStyle` 时插到 pPr 首位。见 `_ensure_keep_next()`。
...
23. **落款被挤到单独一页是常见结尾缺陷**：正文排满（每页余量常不足 1 行）时，最后一页只剩「发文机关署名 + 成文日期」10 来个字，非常扎眼。GB/T 9704-2012 明确允许「当公文排版后所剩空白处不能容下印章或成文日期时，可以采取调整行距、字间距的方法解决，务使印章与正文同处一页」。用 `--fit-signoff` 处置（见「3. 转换文稿」）。**别用硬分页**（把正文最后一段也顶下去会留大片空白），也别为了塞落款就随意改正文内容。
24. **改内容会改变页数，改完必须重新核版**：删掉一两个环节后页数、落款位置都会变（本例删两段后仍 8 页、落款掉到孤页；紧凑处置后回到 7 页）。判定"页数是否合理"要减掉下边距再算余量，见第 21 条。
